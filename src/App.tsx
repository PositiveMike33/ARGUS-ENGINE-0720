...
interface HeaderClockProps {
  formatEstTime: (date: Date, includeSeconds?: boolean) => string;
}

const HeaderClock: React.FC<HeaderClockProps> = ({ formatEstTime }) => {
  ...
  return (
    <span className="text-slate-200 font-semibold">{formatEstTime(time)}</span>
  );
};
...
export default function App() {
  ...
  // --- ÉTATS MCA, ENCLAVE DE SÉCURITÉ ET ENFORCER DE BUDGET JETONS (Opérateur & Sceptique) ---
  const [isMcaEnabled, setIsMcaEnabled] = useState<boolean>(true);
  const [tokenBudget, setTokenBudget] = useState<number>(150000); // 150,000 tokens max par défaut
  const [usedTokens, setUsedTokens] = useState<number>(() => {
    try {
      const cached = localStorage.getItem('argus_used_tokens');
      return cached ? parseInt(cached, 10) : 34500;
    } catch (e) {
      return 34500;
    }
  });
  const [secureEnclaveActive, setSecureEnclaveActive] = useState<boolean>(true);
  
  // Modales d'autorisation MCA et d'avertissement de budget
  const [mcaModalOpen, setMcaModalOpen] = useState<boolean>(false);
  const [mcaPendingFeed, setMcaPendingFeed] = useState<FeedItem | null>(null);
  const [mcaPendingWeights, setMcaPendingWeights] = useState<{ transit: number; safety: number; uncertainty: number } | undefined>(undefined);
  const [budgetExceededModalOpen, setBudgetExceededModalOpen] = useState<boolean>(false);

  // Sauvegarde persistante des jetons utilisés
  useEffect(() => {
    localStorage.setItem('argus_used_tokens', String(usedTokens));
  }, [usedTokens]);
  ...
  const filteredFeedsByTime = useMemo(() => {
    if (timeOffsetMinutes === 0) return feeds;
  ...
  useEffect(() => {
    ...
    }, 15000); // Only update top-level currentTime state every 15 seconds to prevent full-app re-renders
    return () => clearInterval(timer);
  }, []);

  // Animation automatique du replay historique (Play/Pause)
  useEffect(() => {
      ...
      intervalId = setInterval(() => {
        setTimeOffsetMinutes((prev) => {
          if (prev >= 0) {
            ...
            return 0; // stop au temps réel
          }
          return Math.min(0, prev + 5);
        });
      }, 1000);
    }
    return () => {
  ...
  const handleAnalyzeFeed = async (
    item: FeedItem, 
    weights?: { transit: number; safety: number; uncertainty: number },
    skipMca: boolean = false
  ) => {
    // Calcul de l'estimation du coût en jetons basé sur le modèle sélectionné
    const estimateTokenCost = (model: string) => {
      switch (model) {
        case 'gemini-3.5-flash': return 6800;
        case 'gemini-2.5-flash': return 3100;
        case 'gemini-1.5-flash': return 1450;
        default: return 5000;
      }
    };

    const estCost = estimateTokenCost(selectedModel);

    // 1. Sceptique : Budget Token Enforcer
    if (usedTokens + estCost > tokenBudget) {
      setBudgetExceededModalOpen(true);
      return;
    }

    // 2. Opérateur : Intercepteur MCA (Manual Control Authorization)
    if (isMcaEnabled && !skipMca) {
      setMcaPendingFeed(item);
      setMcaPendingWeights(weights);
    ...
    setMcaModalOpen(false); // S'assurer de fermer la modale

    // Consommation du budget de jetons
    setUsedTokens(prev => prev + estCost);

    // 3. COURT-CIRCUIT : Si le quota est déjà épuisé, on ne tente même plus l'appel API
    if (isMockMode) {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col font-sans select-none antialiased">
        ...
        <div className="max-w-7xl mx-auto flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[9px] sm:text-[10px] text-slate-400 font-mono tracking-widest uppercase flex flex-wrap items-center gap-1.5 sm:gap-2">
                <span>Orchestrateur de Transit Multimodal Prédictif</span>
                <span className="text-slate-600 hidden sm:inline">•</span>
                <span className="text-indigo-400 font-semibold bg-indigo-950/40 px-1.5 py-0.5 rounded border border-indigo-900/30">MTR COGNITIF</span>
              </p>
            </div>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-4 sm:gap-6 text-xs font-mono">
            ...
            <div className="bg-slate-950 px-3.5 py-1.5 rounded border border-slate-900 flex items-center gap-2">
              ...
              <HeaderClock formatEstTime={formatEstTime} />
            </div>
            ...
            >
              ...
              <div className="bg-slate-900/60 backdrop-blur-md rounded-xl border border-slate-800 p-4 shadow-xl flex flex-col lg:flex-row lg:items-stretch justify-between gap-5 transition-all hover:border-slate-700/60" id="stm-realtime-monitoring-banner">
                <div className="space-y-1.5 flex-1 flex flex-col justify-between text-left">
                  ...
                  {stmMonitorView === 'METRO' ? (
                    <motion.div 
                      key={`metro-status-${lastStmFetchTime ? lastStmFetchTime.getTime() : 'initial'}`}
                      initial={{ opacity: 0, y: 4 }}
                      animate={{ opacity: 1, y: 0 }}
                      transition={{ duration: 0.5, ease: "easeOut" }}
                      className="flex flex-wrap items-center gap-3 pt-3 lg:pt-0"
                    >
                      {/* Verte */}
                    ...
                    <motion.div 
                      key={`bus-status-${lastStmFetchTime ? lastStmFetchTime.getTime() : 'initial'}`}
                      initial={{ opacity: 0, y: 4 }}
                      animate={{ opacity: 1, y: 0 }}
                      transition={{ duration: 0.5, ease: "easeOut" }}
                      className="flex flex-wrap items-center gap-3 pt-3 lg:pt-0"
              ...
              <div 
                className={`bg-slate-950 rounded-xl border border-slate-900 p-5 space-y-4 transition-all duration-300 ${
                  isGeospatialFullscreen 
                    ? 'fixed inset-0 z-[9999] flex flex-col h-screen rounded-none p-6 overflow-y-auto' 
                    : ''
                }`} 
                id="geospatial-aviation-intelligence-center"
              >
                <div className="flex flex-col sm:flex-row sm:items-center justify-between border-b border-slate-800 pb-3 gap-3 text-left">
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    {/* Bouton Plein écran */}
                    <button
                      onClick={() => setIsGeospatialFullscreen(!isGeospatialFullscreen)}
                      className="px-3.5 py-1.5 rounded-lg text-xs font-mono font-bold transition-all duration-200 border cursor-pointer bg-slate-900 border-indigo-500/30 text-indigo-400 hover:text-indigo-300 hover:border-indigo-500 flex items-center gap-1.5 sm:ml-2"
                      title={isGeospatialFullscreen ? "Quitter le plein écran" : "Agrandir la carte"}
                    >
                      {isGeospatialFullscreen ? <Minimize2 className="w-3.5 h-3.5 animate-pulse" /> : <Maximize2 className="w-3.5 h-3.5" />}
                      <span>{isGeospatialFullscreen ? 'QUITTER PLEIN ÉCRAN' : 'PLEIN ÉCRAN'}</span>
                    </button>
                  </div>
                </div>

                {/* CURSEUR TEMPOREL (TIME-SLIDER) ET MOTEUR DE REPLAY ARGUS */}
                <div className="bg-slate-900/50 border border-slate-900 rounded-xl p-4 flex flex-col md:flex-row items-center justify-between gap-4 text-left">
                  <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 w-full md:w-auto">
                    <div className="flex items-center gap-2">
                      ...
                      <div className="text-left">
                        <h3 className="text-xs font-bold text-slate-200 tracking-wider font-mono">SUPERVISION CHRONOLOGIQUE ARGUS</h3>
                        <p className="text-[10px] text-slate-500 font-mono mt-0.5">CORRÉLATION HISTORIQUE DES INCIDENTS</p>
                      </div>
                    </div>
                    
                    {/* Badge d'état dynamique */}
                    <div className="flex items-center gap-2 font-mono text-[9px] font-bold px-2.5 py-1 rounded-full border bg-slate-950">
                      {timeOffsetMinutes === 0 ? (
                        <>
                          <span className="w-2 h-2 rounded-full bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.6)] animate-pulse" />
                          <span className="text-emerald-400 uppercase">🔴 DIRECT SYNCHRONISÉ</span>
                        </>
                      ) : (
                        <>
                          <span className="w-2 h-2 rounded-full bg-amber-500 shadow-[0_0_8px_rgba(245,158,11,0.6)] animate-pulse" />
                          <span className="text-amber-400 uppercase">⏳ HISTORIQUE SIMULÉ ({Math.abs(timeOffsetMinutes)} MIN)</span>
                        </>
                      )}
                    </div>
                  </div>

                  {/* Contrôleur de Replay + Glissière */}
                  <div className="flex flex-col sm:flex-row items-center gap-4 w-full md:flex-1 md:max-w-xl">
                    <div className="flex items-center gap-2 w-full">
                      ...
                      <button
                        ...
                        title={isPlayingReplay ? "Suspendre la lecture chronologique" : "Lancer la lecture automatique chronologique"}
                      >
                        {isPlayingReplay ? <Pause className="w-3.5 h-3.5" /> : <Play className="w-3.5 h-3.5 ml-0.5" />}
                      </button>
                      ...
                      <div className="flex-1 flex flex-col space-y-1">
                        <input
                          ...
                          onChange={(e) => {
                            setTimeOffsetMinutes(parseInt(e.target.value));
                          ...
                          style={{
                            background: `linear-gradient(to right, rgb(79, 70, 229) 0%, rgb(79, 70, 229) ${((timeOffsetMinutes + 240) / 240) * 100}%, rgb(30, 41, 59) ${((timeOffsetMinutes + 240) / 240) * 100}%, rgb(30, 41, 59) 100%)`
                          }}
                        />
                        <div className="flex justify-between text-[8px] font-mono text-slate-500 px-0.5">
                          ...
                          <span>-2H</span>
                          <span>-1H</span>
                          <span className={timeOffsetMinutes === 0 ? "text-indigo-400 font-bold" : ""}>TEMPS RÉEL</span>
                        </div>
                      </div>
                    </div>
                  </div>

                  {/* Indicateurs Heure Simulée vs Réelle */}
                  <div className="flex flex-col items-end gap-1 font-mono text-[10px] w-full sm:w-auto">
                    <div className="flex items-center gap-1.5">
                      <span className="text-slate-500">HEURE SIMULÉE :</span>
                      <span className={`font-bold ${timeOffsetMinutes === 0 ? 'text-indigo-400' : 'text-amber-400'}`}>
                        {new Date(currentTime.getTime() + timeOffsetMinutes * 60 * 1000).toLocaleTimeString('fr-FR')}
                      </span>
                    </div>
                    {timeOffsetMinutes !== 0 && (
                    ...
                    {timeOffsetMinutes !== 0 && (
                      <button
                        ...
                        className="text-[9px] font-bold text-indigo-400 hover:text-indigo-300 transition-colors duration-150 flex items-center gap-1 mt-1 cursor-pointer"
                      >
                ...
                {activeGeospatialTab === 'gtfs' ? (
                  <div className={`w-full ${isGeospatialFullscreen ? 'flex-1 min-h-[500px]' : 'h-[600px]'}`}>
                  ...
                  <div className={`grid grid-cols-1 lg:grid-cols-12 gap-6 items-stretch ${isGeospatialFullscreen ? 'flex-1 min-h-0' : ''}`}>
                    ...
                    <div className={`lg:col-span-4 flex flex-col ${isGeospatialFullscreen ? 'flex-1 h-full min-h-[450px]' : 'h-[520px]'}`}>
                      <OpenSkyTracker
                        selectedFlight={selectedFlight}
              ...
              <section className="lg:col-span-5 flex flex-col space-y-6">
                <div className="flex-1">
                  <ToTReasoner
                    ...
                    // Nouveaux contrôles MCA & Budget Jetons
                    isMcaEnabled={isMcaEnabled}
                    onToggleMca={setIsMcaEnabled}
                    tokenBudget={tokenBudget}
                    onUpdateTokenBudget={setTokenBudget}
                    usedTokens={usedTokens}
                    onResetUsedTokens={() => setUsedTokens(15000)}
                    secureEnclaveActive={secureEnclaveActive}
                    onToggleSecureEnclave={setSecureEnclaveActive}
                  />
                </div>
      ...
      {/* MODALE MCA : MANUAL CONTROL AUTHORIZATION */}
      <AnimatePresence>
        {mcaModalOpen && mcaPendingFeed && (
          <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-md flex items-center justify-center z-50 p-4">
            <motion.div
              ...
              transition={{ duration: 0.15 }}
              className="bg-slate-900 border border-amber-500/30 rounded-xl max-w-md w-full overflow-hidden shadow-2xl font-mono text-xs text-left"
              id="mca-auth-modal"
            >
              {/* Header */}
              <div className="bg-amber-500/10 border-b border-amber-500/25 px-4 py-3.5 text-amber-400 font-bold flex items-center gap-2">
                <ShieldAlert className="w-4 h-4 text-amber-500 animate-pulse" />
                <span>MANUAL CONTROL AUTHORIZATION (MCA)</span>
              </div>
              
              {/* Content */}
              <div className="p-5 space-y-4">
                <p className="text-[11px] text-slate-300 leading-relaxed font-sans">
                  Conformément au protocole d'ingénierie d'<strong>ARGUS ENGINE</strong>, les traitements de raisonnement complexes récursifs (ToT) doivent faire l'objet d'une validation manuelle explicite par le commandant de mission.
                </p>

                <div className="bg-slate-950/80 p-3 rounded-lg border border-slate-850 space-y-2">
                  <div className="flex justify-between border-b border-slate-900 pb-1.5">
                    <span className="text-slate-500 uppercase text-[9px]">CIBLE TACTIQUE</span>
                    <span className="text-amber-400 font-bold max-w-[200px] truncate">{mcaPendingFeed.title}</span>
                  </div>
                  <div className="flex justify-between border-b border-slate-900 pb-1.5">
                    <span className="text-slate-500 uppercase text-[9px]">TYPE DE FLUX</span>
                    <span className="text-slate-300 uppercase">{mcaPendingFeed.type}</span>
                  </div>
                  <div className="flex justify-between border-b border-slate-900 pb-1.5">
                    <span className="text-slate-500 uppercase text-[9px]">MODÈLE DE RAISONNEMENT</span>
                    <span className="text-indigo-400 font-bold uppercase">{selectedModel}</span>
                  </div>
                  <div className="flex justify-between border-b border-slate-900 pb-1.5">
                    <span className="text-slate-500 uppercase text-[9px]">COÛT ESTIMÉ</span>
                    <span className="text-amber-500 font-bold">
                      {selectedModel === 'gemini-3.5-flash' ? '~6,800' : selectedModel === 'gemini-2.5-flash' ? '~3,100' : '~1,450'} Tokens
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-slate-500 uppercase text-[9px]">ENCLAVE CRYPTO</span>
                    <span className={secureEnclaveActive ? 'text-emerald-400 font-bold' : 'text-red-400'}>
                      {secureEnclaveActive ? '🛡️ SÉCURISÉE (init_enclave.sh)' : '⚠️ VULNÉRABLE (FLUX DIRECT)'}
                    </span>
                  </div>
                </div>

                <div className="p-2.5 rounded bg-slate-950/40 border border-slate-850 text-[10px] text-slate-400 leading-normal font-sans">
                  * En cliquant sur "APPROUVER", vous autorisez l'agent autonome à allouer le budget de jetons nécessaire et à ouvrir un canal sécurisé vers l'enclave d'ingestion.
                </div>
              </div>

              {/* Actions */}
              <div className="bg-slate-950/50 border-t border-slate-900 px-4 py-3 flex items-center justify-end gap-2.5">
                <button
                  type="button"
                  onClick={() => {
                    setMcaModalOpen(false);
                    setMcaPendingFeed(null);
                    setMcaPendingWeights(undefined);
                  }}
                  className="px-3.5 py-1.5 bg-slate-950 hover:bg-slate-900 border border-slate-800 rounded text-slate-400 hover:text-slate-300 transition-colors font-bold cursor-pointer"
                >
                  ANNULER
                </button>
                <button
                  ...
                  onClick={() => {
                    if (mcaPendingFeed) {
                      handleAnalyzeFeed(mcaPendingFeed, mcaPendingWeights, true);
                    }
                  }}
                  className="px-4 py-1.5 bg-amber-500 hover:bg-amber-400 text-slate-950 rounded font-bold transition-all shadow-md flex items-center gap-1.5 cursor-pointer"
                >
                  <Check className="w-3.5 h-3.5" />
                  <span>APPROUVER LA MISSION ToT</span>
                </button>
              </div>
            </motion.div>
          </div>
      ...
      {/* MODALE SCEPTIQUE : CRITICAL BUDGET EXCEEDED */}
      <AnimatePresence>
        {budgetExceededModalOpen && (
          <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-md flex items-center justify-center z-50 p-4">
            <motion.div
              initial={{ opacity: 0, scale: 0.95 }}
              animate={{ opacity: 1, scale: 1 }}
              exit={{ opacity: 0, scale: 0.95 }}
              transition={{ duration: 0.15 }}
              className="bg-slate-900 border border-red-500/30 rounded-xl max-w-md w-full overflow-hidden shadow-2xl font-mono text-xs text-left"
              id="budget-exceeded-modal"
            >
              {/* Header */}
              <div className="bg-red-500/10 border-b border-red-500/25 px-4 py-3.5 text-red-400 font-bold flex items-center gap-2">
                <AlertTriangle className="w-4 h-4 text-red-500 animate-pulse" />
                <span>BLOCAGE CRITIQUE : BUDGET SESSION ÉPUISÉ</span>
              </div>
              
              {/* Content */}
              <div className="p-5 space-y-4">
                <p className="text-[11px] text-slate-300 leading-relaxed font-sans">
                  Le filtre de restriction financière d'<strong>ARGUS ENGINE</strong> (conçu par l'expert Sceptique) a bloqué cette requête car la consommation estimée dépasse le plafond de jetons restant pour votre session active.
                </p>

                <div className="bg-slate-950/80 p-3 rounded-lg border border-slate-850 space-y-2">
                  <div className="flex justify-between border-b border-slate-900 pb-1.5">
                    <span className="text-slate-500 uppercase text-[9px]">JETONS CONSOMMÉS</span>
                    <span className="text-slate-300 font-bold">{usedTokens.toLocaleString()}</span>
                  </div>
                  <div className="flex justify-between border-b border-slate-900 pb-1.5">
                    <span className="text-slate-500 uppercase text-[9px]">PLAFOND ALLOUÉ</span>
                    <span className="text-red-400 font-bold">{tokenBudget.toLocaleString()}</span>
                  </div>
                  <div className="flex justify-between border-b border-slate-900 pb-1.5">
                    <span className="text-slate-500 uppercase text-[9px]">MODÈLE ACTIF</span>
                    <span className="text-indigo-400 uppercase font-bold">{selectedModel}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-slate-500 uppercase text-[9px]">COÛT ESTIMÉ OPÉRATION</span>
                ...
                <div className="p-3 bg-red-950/20 border border-red-900/40 rounded text-[10px] text-red-300 leading-normal font-sans">
                  💡 <strong>Comment débloquer ?</strong><br />
                  1. Augmentez le budget de jetons session à l'aide des boutons <span className="bg-slate-950 px-1 border border-slate-800 rounded font-mono">+25k</span> dans l'enclave ToT.<br />
                  2. Basculez sur le modèle <strong>gemini-2.5-flash</strong> (-70% de coût) ou <strong>gemini-1.5-flash</strong> (-85% de coût) pour minimiser la consommation financière de l'arbre ToT.
                </div>
              </div>
              ...
              <div className="bg-slate-950/50 border-t border-slate-900 px-4 py-3 flex items-center justify-end gap-2.5">
                <button
                  type="button"
                  onClick={() => setBudgetExceededModalOpen(false)}
                  className="px-4 py-1.5 bg-slate-950 hover:bg-slate-900 border border-slate-800 rounded text-slate-400 hover:text-slate-200 transition-colors font-bold cursor-pointer"
                >
                  FERMER
                </button>
                <button
                  type="button"
                  onClick={() => {
                    setTokenBudget(prev => prev + 50000);
                    setBudgetExceededModalOpen(false);
                  }}
                  className="px-4 py-1.5 bg-indigo-600 hover:bg-indigo-500 text-white rounded font-bold transition-all shadow-md flex items-center gap-1.5 cursor-pointer"
                >
                  <span>ALLOUER +50K TOKENS</span>
                </button>
              </div>
            </motion.div>
          </div>
