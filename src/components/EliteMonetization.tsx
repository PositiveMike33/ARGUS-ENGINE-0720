  ...
  -d '{
    ...
    "details": "Substation energy surge detected at Berri-UQAM..."
            ...
            <div className="space-y-3">
              ...
              <div className="max-h-[160px] overflow-y-auto rounded-lg border border-slate-800/80 bg-slate-950/60 p-2.5 font-mono text-[10px] relative">
                {isLoadingLogs && (
                  <motion.div 
                    initial={{ opacity: 0 }}
                    animate={{ opacity: 1 }}
                    exit={{ opacity: 0 }}
                    className="absolute inset-0 bg-slate-950/40 backdrop-blur-[1px] flex items-center justify-center z-10 rounded-lg"
                  >
                    <div className="flex items-center gap-2 px-3 py-1.5 bg-slate-900/90 border border-slate-800 rounded-md text-emerald-400 font-bold shadow-lg text-[9px]">
                      <RefreshCw className="w-3 h-3 animate-spin" />
                      <span>SYNCHRONISATION DES FLUX...</span>
                    </div>
                  </motion.div>
                )}
                    ...
                    {filteredLogs.length > 0 ? (
                      filteredLogs.map((log) => (
                        <motion.div 
                          ...
                          className="flex items-center justify-between border-b border-slate-900/60 pb-1.5 last:border-0 last:pb-0 overflow-hidden"
                        >
                      ...
                      >
                        Aucun log correspondant au filtre ou enregistré.
                      </motion.p>
                    )}
                  </AnimatePresence>
                </div>
