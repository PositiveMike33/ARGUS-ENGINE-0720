...
interface ToTReasonerProps {
  ...
  // MCA & Token Budget Props (Opérateur & Sceptique)
  isMcaEnabled?: boolean;
  onToggleMca?: (val: boolean) => void;
  tokenBudget?: number;
  onUpdateTokenBudget?: (val: number) => void;
  usedTokens?: number;
  onResetUsedTokens?: () => void;
  secureEnclaveActive?: boolean;
  onToggleSecureEnclave?: (val: boolean) => void;
}
...
export const ToTReasoner: React.FC<ToTReasonerProps> = ({
  ...
  // Destructuring security controls with safe fallbacks
  isMcaEnabled = true,
  onToggleMca,
  tokenBudget = 150000,
  onUpdateTokenBudget,
  ...
  return (
    ...
    >
      ...
      {/* SECTION SÉCURITÉ MCA & BUDGET JETONS (Opérateur & Sceptique) */}
      <div className="bg-slate-950 border-b border-slate-900/80 p-3.5 space-y-3 font-mono text-xs">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-slate-900 pb-1.5">
          <div className="flex items-center gap-1.5 text-slate-300 font-bold">
            <ShieldAlert className="w-4 h-4 text-amber-500 animate-pulse" />
            <span>ENCLAVE DE SÉCURITÉ & BUDGET FINANCIER (MCA)</span>
          </div>
          {secureEnclaveActive && (
            <span className="text-[9.5px] px-2 py-0.5 rounded bg-emerald-950/85 text-emerald-400 border border-emerald-900/40 font-bold flex items-center gap-1.5 self-start sm:self-auto">
              <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-pulse" />
              LOCK : ENCLAVE ACTIVE (init_enclave.sh)
            </span>
          )}
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {/* MCA Toggle */}
          <div className="flex flex-col justify-between bg-slate-900/60 p-2.5 rounded-lg border border-slate-800/80">
            <div className="flex items-center justify-between">
              <span className="text-[10px] text-slate-400">Manual Control Auth (MCA)</span>
              <span className={`text-[9px] font-bold px-1.5 py-0.5 rounded ${
                isMcaEnabled ? 'bg-amber-950 text-amber-400 border border-amber-900/40' : 'bg-slate-950 text-slate-500 border border-slate-800'
              }`}>
                {isMcaEnabled ? 'REQUIS' : 'AUTONOME'}
              </span>
            </div>
            <p className="text-[9.5px] text-slate-500 font-sans leading-tight my-1.5">
              Empêche tout traitement ToT non validé manuellement.
            </p>
            <button
              type="button"
              onClick={() => onToggleMca && onToggleMca(!isMcaEnabled)}
              className={`w-full py-1 rounded text-[10px] font-bold border transition-all cursor-pointer ${
                isMcaEnabled 
                  ? 'bg-amber-600/15 hover:bg-amber-600/25 text-amber-400 border-amber-500/30' 
                  : 'bg-slate-950 hover:bg-slate-900 text-slate-400 border-slate-800'
              }`}
            >
              {isMcaEnabled ? 'DÉSACTIVER APPROBATION' : 'ACTIVER APPROBATION MCA'}
            </button>
          </div>
          ...
          <div className="flex flex-col justify-between bg-slate-900/60 p-2.5 rounded-lg border border-slate-800/80">
            <div className="flex items-center justify-between text-[10px]">
              <span className="text-slate-400">Budget Session</span>
              <span className="font-bold text-slate-300">
                {usedTokens.toLocaleString()} / {tokenBudget.toLocaleString()} TOKENS
              </span>
            </div>
            {/* Progress bar */}
            <div className="w-full bg-slate-950 h-1.5 rounded-full overflow-hidden border border-slate-850 my-2">
              <div 
                className={`h-full rounded-full transition-all duration-500 ${
                  (usedTokens / tokenBudget) > 0.85 ? 'bg-red-500 animate-pulse' :
                  (usedTokens / tokenBudget) > 0.6 ? 'bg-amber-500' : 'bg-indigo-500'
                }`}
                style={{ width: `${Math.min(100, (usedTokens / tokenBudget) * 100)}%` }}
              />
            </div>
            <div className="flex items-center justify-between text-[9px]">
              <span className="text-slate-500">
                Estimé : {(usedTokens * 0.000015).toFixed(3)}$ USD
              </span>
              <div className="flex gap-1.5">
                <button
                  type="button"
                  onClick={() => onUpdateTokenBudget && onUpdateTokenBudget(Math.max(50000, tokenBudget - 25000))}
                  className="px-1.5 py-0.5 bg-slate-950 hover:bg-slate-900 border border-slate-800 text-slate-400 rounded hover:text-slate-300 font-bold transition-all cursor-pointer"
                  title="Diminuer le budget"
                >
                  -25k
                </button>
                <button
                  type="button"
                  onClick={() => onUpdateTokenBudget && onUpdateTokenBudget(tokenBudget + 25000)}
                  className="px-1.5 py-0.5 bg-slate-950 hover:bg-slate-900 border border-slate-800 text-slate-400 rounded hover:text-slate-300 font-bold transition-all cursor-pointer"
          ...
          {/* Enclave Network Encryption */}
          <div className="flex flex-col justify-between bg-slate-900/60 p-2.5 rounded-lg border border-slate-800/80">
            <div className="flex items-center justify-between text-[10px]">
              <span className="text-slate-400">Isolation Réseau</span>
              <span className={`text-[9px] font-bold px-1.5 py-0.5 rounded ${
                secureEnclaveActive ? 'bg-emerald-950 text-emerald-400 border border-emerald-900/40' : 'bg-red-950 text-red-400 border border-red-900/40'
              }`}>
                {secureEnclaveActive ? 'BLINDÉE' : 'VULNÉRABLE'}
              </span>
            </div>
            <p className="text-[9.5px] text-slate-500 font-sans leading-tight my-1.5">
              Force le tunneling via le protocole d'isolement réseau d'ARGUS.
            </p>
            <button
              type="button"
              onClick={() => onToggleSecureEnclave && onToggleSecureEnclave(!secureEnclaveActive)}
              className={`w-full py-1 rounded text-[10px] font-bold border transition-all cursor-pointer ${
                secureEnclaveActive 
            ...
            >
              {secureEnclaveActive ? 'COUPER L\'ISOLATION' : 'ISOLER LE FLUX (ENCLAVE)'}
            </button>
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 p-5 overflow-y-auto space-y-5">
        ...
        {analysisError ? (
          ...
          <div className="space-y-5 animate-fade-in">
            ...
            {/* Memory Bank - Vector Long-term Semantic Decisions Match */}
            <div className="p-4 rounded-lg bg-slate-950 border border-indigo-900/40 space-y-3 shadow-md">
              <div className="flex items-center justify-between border-b border-indigo-900/30 pb-2">
                <span className="text-[11px] font-mono font-bold text-indigo-400 uppercase tracking-widest flex items-center gap-1.5">
                  <Brain className="w-4 h-4 text-indigo-400 animate-pulse" />
                  <span>BANQUE DE MÉMOIRE LONG TERME SÉMANTIQUE (VÉCTOR DB)</span>
                </span>
                <span className="text-[9px] font-mono px-2 py-0.5 rounded bg-indigo-950/50 text-indigo-300 border border-indigo-900/40 uppercase tracking-wider">
                  MÉMOIRE ACTIVE ({result.relatedMemories?.length || 0})
                </span>
              </div>

              {result.relatedMemories && result.relatedMemories.length > 0 ? (
                <div className="space-y-3">
                  <p className="text-[10px] font-mono text-slate-400 leading-normal mb-1">
                    Le moteur a récupéré de manière sémantique les décisions passées les plus similaires pour guider l'opérateur et garantir la cohérence des actions :
                  </p>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    {result.relatedMemories.map((memory) => {
                      const relevanceScore = Math.round(memory.similarity * 100);
                      return (
                        <div key={memory.id} className="p-3 bg-slate-900/60 rounded border border-slate-850/80 space-y-2 text-xs flex flex-col justify-between">
                          <div>
                            <div className="flex items-center justify-between mb-1">
                              <span className="text-[10.5px] font-mono font-bold text-slate-100 truncate max-w-[65%]" title={memory.feedTitle}>
                                {memory.feedTitle}
                              </span>
                              
                              {/* Confidence / Relevance visual indicator */}
                              <div className="flex items-center gap-1.5">
                                <div className="w-10 h-1.5 bg-slate-950 rounded-full overflow-hidden border border-slate-800">
                                  <div 
                                    className={`h-full rounded-full ${
                                      relevanceScore >= 85 ? 'bg-emerald-500' :
                                      relevanceScore >= 70 ? 'bg-indigo-500' :
                                      'bg-amber-500'
                                    }`}
                                ...
                                <span className={`text-[9px] font-mono font-bold px-1.5 py-0.5 rounded ${
                                  relevanceScore >= 85 ? 'bg-emerald-950/85 text-emerald-400 border border-emerald-900/30' :
                                  relevanceScore >= 70 ? 'bg-indigo-950/85 text-indigo-400 border border-indigo-900/30' :
                            ...
                            <div className="flex items-center gap-2 text-[9px] font-mono text-slate-500 mb-2">
                              <span className="px-1 py-0.2 rounded bg-slate-950 border border-slate-800">{memory.feedType}</span>
                              <span>•</span>
                              <span>Entropie : {memory.entropyScore} bits</span>
                              {memory.createdAt && (
                                <>
                                  <span>•</span>
                                  <span>{new Date(memory.createdAt).toLocaleDateString('fr-CA')}</span>
                                </>
                              )}
                            </div>

                            <p className="text-[11px] text-slate-300 font-sans italic leading-relaxed bg-slate-950/40 p-2 rounded border border-slate-900/50">
                              "{memory.finalDecision.length > 200 ? `${memory.finalDecision.substring(0, 197)}...` : memory.finalDecision}"
                            </p>
                          </div>
                        </div>
                      );
                ...
                <div className="p-3 bg-slate-900/30 rounded border border-slate-900 text-center py-4">
                  <Database className="w-5 h-5 text-slate-600 mx-auto mb-1" />
                  <p className="text-[10.5px] font-mono text-slate-500">
                    Aucun précédent décisionnel similaire détecté dans la base sémantique.
                  </p>
                </div>
              )}
            </div>
