...
export function ApiTester({ user }: ApiTesterProps) {
  ...
  const handleClearOldLogs = async () => {
    ...
    try {
      ...
      const response = await fetch('/api/telemetry/clear-old-logs', {
        method: 'POST',
        headers
      });
      ...
      if (response.ok && data.success) {
        setClearResult({
          success: true,
          message: data.message || 'Purge terminée.',
          deletedFirestoreCount: data.deletedFirestoreCount,
          deletedPostgresCount: data.deletedPostgresCount,
          deletedMemoryCount: data.deletedMemoryCount
        });
      } else {
        setClearResult({
          success: false,
          error: data.error || 'Erreur lors de la purge.'
        });
      }
    } catch (err: any) {
      setClearResult({
        success: false,
        error: err.message || 'La connexion au serveur a échoué.'
      });
    } finally {
      setIsClearing(false);
    }
    ...
    <div className="bg-slate-900/40 border border-slate-900 rounded-xl p-5 space-y-6" id="api-tester-suite">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        <div className="lg:col-span-5 space-y-4">
          ...
          {/* Firestore Telemetry Database Optimization */}
          <div className="bg-slate-950 rounded-xl border border-slate-900 p-4 space-y-3 text-left text-xs font-mono">
            <h3 className="text-[11px] font-bold text-slate-300 uppercase tracking-wider pb-1.5 border-b border-slate-900 flex items-center gap-1.5">
            ...
            <p className="text-[10px] text-slate-400 leading-normal">
              Purgez les logs de télémétrie obsolètes vieux de plus de 30 jours pour libérer de l'espace et optimiser les performances de Firestore.
            </p>
            <button
              onClick={handleClearOldLogs}
              disabled={isClearing}
              className="w-full py-2 bg-rose-950/40 hover:bg-rose-900/30 text-rose-400 border border-rose-800/60 rounded-lg font-bold text-[11px] transition-all duration-150 flex items-center justify-center gap-2 cursor-pointer disabled:opacity-50 font-sans"
            >
              {isClearing ? (
                <>
                ...
                <>
                  ...
                  <span>SUPPRIMER LES LOGS OBSOLÈTES (&gt; 30 J)</span>
                </>
              )}
            </button>

            {clearResult && (
              <div className={`p-2 rounded border text-[10px] leading-normal ${
                clearResult.success 
                ...
                {clearResult.success ? (
                  <div className="space-y-1">
                    <div className="font-bold flex items-center gap-1">
                      ...
                      <span>Optimisation réussie !</span>
                    </div>
                    <p className="text-[9.5px] text-slate-400">
                      {clearResult.message}
                    </p>
                    <div className="grid grid-cols-3 gap-1 mt-1.5 pt-1.5 border-t border-emerald-900/30 text-center font-mono text-[9px] text-slate-400">
                      <div>
                        <span className="block font-bold text-slate-200">{clearResult.deletedFirestoreCount}</span>
                        <span>Firestore</span>
                      </div>
                      <div>
                        <span className="block font-bold text-slate-200">{clearResult.deletedPostgresCount}</span>
                        <span>Postgres</span>
                      </div>
                      <div>
                        <span className="block font-bold text-slate-200">{clearResult.deletedMemoryCount}</span>
                        <span>Mémoire</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex items-start gap-1.5">
                    ...
                    <div>
                      <span className="font-bold block">Échec de l'optimisation</span>
                      <span className="text-[9.5px] text-slate-400">{clearResult.error}</span>
                    </div>
                  </div>
                )}
              </div>
