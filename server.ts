...
// Helper to generate embedding using gemini-embedding-2-preview
async function generateEmbedding(text: string): Promise<number[] | null> {
  ...
  try {
    ...
    console.warn('[Vector Memory] Failed to generate embedding:', err);
  }
  return null;
}
...
// REST Endpoint: Clear telemetry logs older than 30 days (Firestore, Cloud SQL and Memory)
app.post('/api/telemetry/clear-old-logs', optionalAuth, async (req: any, res) => {
  const start = Date.now();
  try {
    ...
    let deletedPostgresCount = 0;
    let deletedMemoryCount = 0;

    // 1. Clean up local in-memory logs
    const initialMemoryCount = apiLogs.length;
    for (let i = apiLogs.length - 1; i >= 0; i--) {
      const logTime = new Date(apiLogs[i].timestamp);
      if (logTime < cutoffDate) {
        apiLogs.splice(i, 1);
      }
    }
    deletedMemoryCount = initialMemoryCount - apiLogs.length;

    // 2. Clean up authenticated user's logs
    if (req.user) {
      const dbUser = await getOrCreateUser(req.user.uid, req.user.email || 'operator@argus.io');
      
      // Clean up in Firestore using firebase-admin SDK
      try {
        const logsRef = adminDb.collection('telemetry_logs');
        ...
        if (!snapshot.empty) {
          ...
          await batch.commit();
          deletedFirestoreCount = snapshot.size;
        }
      } catch (firestoreErr) {
        console.error('Error clearing old telemetry logs in Firestore:', firestoreErr);
      }

      // Clean up in PostgreSQL (Cloud SQL)
      try {
        const result = await db.delete(telemetryLogs)
          .where(lte(telemetryLogs.createdAt, cutoffDate));
        deletedPostgresCount = result ? (result as any).rowCount || 0 : 0;
      } catch (dbErr) {
        console.error('Error clearing old telemetry logs in Cloud SQL:', dbErr);
      }

      await logAPI('/api/telemetry/clear-old-logs', 200, start, 100, dbUser.id);
    } else {
      // In guest/preview mode, we can still perform a global Firestore purge for logs without userId or mock logs
      try {
        const logsRef = adminDb.collection('telemetry_logs');
        const snapshot = await logsRef
          .where('timestamp', '<', cutoffStr)
        ...
        console.error('Error in guest clear of old telemetry logs in Firestore:', firestoreErr);
      }

      await logAPI('/api/telemetry/clear-old-logs', 200, start, 100);
    }

    res.json({
      success: true,
      message: 'Les logs de télémétrie obsolètes (de plus de 30 jours) ont été supprimés avec succès pour optimiser Firestore.',
      deletedFirestoreCount,
      deletedPostgresCount,
      deletedMemoryCount,
      cutoffDate: cutoffStr
    });
  } catch (error: any) {
    console.error('Error clearing telemetry logs:', error);
    res.status(500).json({ 
      success: false, 
      error: error.message || 'Une erreur interne est survenue lors du nettoyage des logs.' 
    });
  }
});
...
let isStmFetchingInBackground = false;
...
async function fetchRealStmAlerts(): Promise<{ success: boolean; format?: string; data?: any; error?: string; bytes?: number }> {
  const apiKey = process.env.STM_API_CLIENT_ID;
  const apiSecret = process.env.STM_API_CLIENT_SECRET || '';
...
app.get('/api/stm/realtime', optionalAuth, async (req: any, res) => {
  ...
  // 1. If we have a fresh valid cached response, serve it instantly
  ...
  // 2. Otherwise, serve stale cache or fallback instantly to achieve 0ms latency
  const dataToServe = cachedStmResponse ? cachedStmResponse.data : generateStmFallback();
  const groundedToServe = cachedStmResponse ? cachedStmResponse.grounded : false;
  const tsToServe = cachedStmResponse ? cachedStmResponse.timestamp : new Date().toISOString();

  res.json({
    success: true,
    data: dataToServe,
    grounded: groundedToServe,
    timestamp: tsToServe,
    cached: true,
    apiKeyActive: apiKeyActive
  });
  await logAPI('/api/stm/realtime', 200, start, JSON.stringify(dataToServe).length, dbUser?.id);

  // 3. Fire-and-forget asynchronous background sync to fetch and cache actual STM alerts
  if (!isStmFetchingInBackground && (!lastStmErrorTime || (now - lastStmErrorTime >= STM_COOLDOWN_TTL))) {
    isStmFetchingInBackground = true;
    (async () => {
      try {
        console.log('[STM Telemetry] Starting asynchronous background update...');
...
And any other major transit network alerts. Map each line's status to either "normal", "delay", or "interrupted". Provide details and write all messages/alert titles/descriptions in French (fr-CA) so they match the operator console language. Ensure the response format strictly complies with the requested JSON schema.`;
          ...
          lastStmErrorTime = 0;
          console.log('[STM Telemetry] Background update completed successfully.');
        } else {
          cachedStmResponse = {
            data: generateStmFallback(),
            grounded: false,
            timestamp: new Date().toISOString()
          };
          cachedStmTimestamp = Date.now();
...
app.post('/api/tot/analyze', optionalAuth, async (req: any, res) => {
  ...
  try {
    ...
    // Vector Memory: Search for similar past ToT analyses (Long-term Memory context)
    let memoryPromptContext = '';
    let queryEmbedding: number[] | null = null;
    let relatedMemoriesList: any[] = [];

    try {
      const queryText = `Secteur: ${feed.type}. Alerte: ${feed.title}. Détails: ${feed.details}.`;
      queryEmbedding = await generateEmbedding(queryText);

      if (queryEmbedding) {
        // Tenter d'interroger Pinecone d'abord
        relatedMemoriesList = await querySimilarToTAnalyses(queryEmbedding, 3);

        // Si Pinecone n'est pas configuré ou ne retourne aucun résultat, repli sur notre stockage PostgreSQL local/mémoire
        if (relatedMemoriesList.length === 0) {
          let pastDecisions: any[] = [];
          if (req.user) {
            const dbUser = await getOrCreateUser(req.user.uid, req.user.email || 'operator@argus.io');
            pastDecisions = await db.select()
              .from(decisions)
              .where(eq(decisions.userId, dbUser.id))
              .orderBy(desc(decisions.createdAt));
          } else {
            pastDecisions = decisionArchive;
          }

          const matches = [];
          for (const d of pastDecisions) {
            if (d.embedding && Array.isArray(d.embedding)) {
              const score = cosineSimilarity(queryEmbedding, d.embedding as number[]);
              matches.push({
                id: d.id,
                feedTitle: d.feedTitle || '',
                feedType: d.feedType || 'STM',
                finalDecision: d.finalDecision || '',
                entropyScore: d.entropyScore || 0,
                similarity: score,
          ...
          relatedMemoriesList = matches
            .sort((a, b) => b.similarity - a.similarity)
            .slice(0, 3);
        }

        if (relatedMemoriesList.length > 0) {
          memoryPromptContext = `\n\n[CONTEXTE DE MÉMOIRE À LONG TERME (ANALYSES ToT SIMILAIRES RETROUVÉES DANS VOTRE BASE DE DONNÉES VECTORIELLE)]:
Voici les analyses d'alertes ToT antérieures les plus sémantiquement similaires trouvées pour vous inspirer et assurer la cohésion et conformité décisionnelle :
` + relatedMemoriesList.map((m, idx) => {
            return `Mémoire #${idx + 1} (Score de similarité sémantique: ${(m.similarity * 100).toFixed(1)}%):
- Type d'alerte: [${m.feedType}] ${m.feedTitle}
- Score d'entropie quantique calculé: ${m.entropyScore}
- Décision Finale (D.U.R.): ${m.finalDecision}`;
          }).join('\n\n') + `\n\nVeuillez incorporer ou faire référence sémantiquement à cette mémoire à long terme si elle est pertinente pour enrichir, stabiliser et accélérer la cohésion de vos recommandations.`;
        }
      }
    } catch (memErr) {
      console.warn('[Vector Memory] Failed to fetch or calculate similarities:', memErr);
    }
...
3. "Branche 3 : ROI & Impact Économique" (Evaluating commercial asset valuations, contractual SLA fines, supply chain continuity, SaaS Argus-DaaS licensing optimization, and premium Stripe billing triggers)

INTEGRATION INTELLIGENTE SAAS (COMMERCIALISATION) :
La "Branche 3 : ROI & Impact Économique" et la "finalDecision" doivent obligatoirement intégrer l'analyse de rentabilité de la licence SaaS Argus-DaaS. Détaillez comment un abonné commercial de niveau Entreprise bénéficie de la résolution, des alertes SMS Twilio prioritaires, et des rapprochements de facturation automatisés sous Stripe afin de démontrer la valeur continue de la version payante de l'orchestrateur.
...
Ensure the finalDecision is written in French and encapsulates the CCTV_AGENT protocol findings.`;
        ...
        // Inject Long-term Vector Memory context if available
        if (memoryPromptContext) {
          prompt += memoryPromptContext;
        }
...
`;
    ...
    // Attach generated embedding to the decision result for long-term vector memory
    if (queryEmbedding) {
      result.embedding = queryEmbedding;
    } else {
      try {
        const queryText = `Secteur: ${feed.type}. Alerte: ${feed.title}. Détails: ${feed.details}.`;
        result.embedding = await generateEmbedding(queryText);
      } catch (embErr) {
        console.warn('[Vector Memory] On-the-fly embedding generation failed:', embErr);
      }
    }

    // Attach semantic long-term memory matches for the client
    result.relatedMemories = relatedMemoriesList;

    // Asynchronously upsert new decision to Pinecone if embedding exists
    if (result.embedding) {
      upsertToTAnalysis(
        result.id,
        result.feedTitle,
        ...
        result.entropyScore,
        result.embedding
      ).catch(err => console.warn('[Vector Memory] Pinecone async upsert failed:', err));
    }
...
app.get('/api/stm/gtfs-rt', optionalAuth, async (req: any, res) => {
  ...
  try {
    ...
    await logAPI('/api/stm/realtime', 200, start, buffer.byteLength, dbUser?.id);
  } catch (err: any) {
    ...
    await logAPI('/api/stm/realtime', 500, start, 50, dbUser?.id);
  }
});
...
let isOpenSkyFetchingInBackground = false;
...
app.get(['/api/opensky/flights', '/api/vectors/live'], optionalAuth, async (req: any, res) => {
  ...
  try {
    ...
    // 1. If we have fresh cache, return it instantly
    ...
    // 2. Otherwise, serve stale cached flights or mock flights instantly to prevent any blocking latency
    let flightsToServe: any[] = [];
    let sourceToServe = '';

    if (cachedOpenSkyFlights && cachedOpenSkyFlights.length > 0) {
      // Simulate slight movement for cached flights to keep them active
      const timeSec = now / 1000;
      flightsToServe = cachedOpenSkyFlights.map((f: any, idx: number) => {
      ...
      sourceToServe = 'OpenSky Network (Interpolated Fallback)';
    } else {
      flightsToServe = getMockFlights(now);
      sourceToServe = 'ARGUS Active Flight Simulation (Fallback)';
    }

    res.json({
      success: true,
      source: sourceToServe,
      flights: flightsToServe
    });
    await logAPI(currentPath, 200, start, JSON.stringify(flightsToServe).length, dbUser?.id);

    // 3. Fire-and-forget asynchronous background task to fetch live OpenSky data
    if (!isOpenSkyFetchingInBackground) {
      isOpenSkyFetchingInBackground = true;
      (async () => {
        try {
          console.log('[OpenSky API] Refreshing satellite feed in background...');
          const lamin = 44.0;
          const lomin = -76.0;
          const lamax = 47.5;
          const lomax = -71.0;
          ...
          const controller = new AbortController();
          const timeoutId = setTimeout(() => controller.abort(), 25000); // 25 seconds timeout to prevent premature aborts under slow network conditions
          ...
          console.log('[OpenSky API] Background refresh completed successfully. Flights found:', limitedFlights.length);
        } catch (err: any) {
          // Silent automatic failover to high-fidelity simulated trajectories to guarantee continuity
          if (!cachedOpenSkyFlights || cachedOpenSkyFlights.length === 0) {
            cachedOpenSkyFlights = getMockFlights(Date.now());
          } else {
            const nowTime = Date.now();
            const timeSec = nowTime / 1000;
            cachedOpenSkyFlights = cachedOpenSkyFlights.map((f: any, idx: number) => {
              const rad = (f.heading * Math.PI) / 180;
              const offset = Math.sin(timeSec / 30 + idx) * 0.005;
              return {
                ...f,
                longitude: parseFloat((f.longitude + offset * Math.sin(rad)).toFixed(5)),
                latitude: parseFloat((f.latitude + offset * Math.cos(rad)).toFixed(5)),
          ...
          // Reset cache time to prevent immediate re-hammering of the restricted external API
          lastOpenSkyFetchTime = Date.now();
        } finally {
          isOpenSkyFetchingInBackground = false;
        }
      })();
    }

  } catch (outerErr: any) {
    console.error('[OpenSky Router] Critical outer error:', outerErr);
  }
});
